...
Sound_E1:				  
		move.b	#$B6,d0	; FM3/6 panning
		move.b	#$C0,d1	; enable both channels
...
PlayPCM_Loop:	  
		...
		move.w	#SegaPCMPitch,d0		; write the pitch ($14 in this case) to d0 
		...
		jsr	(Jpad_Read).w			; read only the first joypad port. It's important that we do NOT do the two ports, we don't have the cycles for that 
...
loc_7297C:
		...
		blo.s	@psgsendvol	; if not, branch
...
Music93:	incbin	"sound\Mus93 - Get Emerald.bin"
	...
	if LevSelMus=1
Music94:	incbin	"sound\Mus94 - Level Select.bin"
		even
	endc
; ---------------------------------------------------------------------------
...
; END OF ROM should be after "INCLUDE"'ing this ASM