...
SpinDashActive: = 1
	SpinDashDustActive: = SpinDashActive*MoveLamppostVRAM*UseDMAQueue*1	; activates animated dust sprite when Spin Dashing
		SpinDashNoDustUnderwater: = SpinDashDustActive*0	; disables dust while underwater
...
; Date: 2011-02-11 (Feb)
InstaShieldActive: = UseDMAQueue*0
...
; Date: 2011-04-19 (Apr)
BlocksInROM: = 1
; Notes: Seems to cause issues with older versions of Gens.
...
HUDInSpecialStage: = DynamicSpecialStageWalls*1
	HUDInSpecialStageType: = 1	; 0=normal; 1=score not shown; 2=score & time not shown; 3=rings only
...
TimeLimitInSpecialStage: = HUDInSpecialStage*TimeRingsFlashFix*1
	...
	TimeLimitInSpecialStageStart: = $000A0000	; time on the clock to start with when Special Stage is entered. Format: $000MSSCC
...
; Date: 2011-03-02 (Mar)
CustomDemoDelay: = 0
	CustomDemoDelayTime: = $178	; delay until demo is played; default is $178; use -1 for never (will return to Sega Screen eventually, though).
...
; Function: Makes ring timers seperate for each ring instance.
; Date: 2015-10-27 (Oct)
FixedRingTimers: = 1

; Name: Comper Decompression
; Function: Includes the Comper decompression subroutine.
; Date: 2015-10-29 (Oct)
CompDecActive: = 1

; Name: Ported S2 Art Loader
; Function: Ports S2's art loader which uses Kosinski compression and is an overall performance improvement at the cost of a worse compression ratio.
; Date: 2015-10-27 (Oct)
; Depends On: Use DMA Queue
S2ArtLoader: = UseDMAQueue*1
	CompInsteadOfKos: = S2ArtLoader*CompDecActive*1	; repalces Kosinksi with Comper compression for even faster results, depends on Comper Decompression

; Name: Corrected Fading
; Function: Corrects the fade in/out subroutines so they do thier job correctly.
; Date: 2015-10-27 (Oct)
CorrectedFading: = 1

; Name: Faster ObjectMove
; Function: Cuts down the amount of code in the ObjectMove (ObjectFall and SpeedToPos) subroutines thus making them faster.
; Date: 2015-10-27 (Oct)
FasterObjMove: = 1

; Name: Underwater Ring Physics
; Function: Makes underwater physics also apply to rings.
; Date: 2015-10-27 (Oct)
UnderwaterRingPhysics: = 1

; Name: New Zone Patches
; Function: Patches some things to prepare the game for a brand new zone!
; Date: 2015-10-27 (Oct)
NewZonePatches: = 1

; Name: Extend Sprite Limit
; Function: Extends the sprite limit, what did you expect?
; Date: 2015-10-27 (Oct)
ExtendSpriteLimit: = 1

; Name: Disable Points
; Function: Disables the points which fly out of enemies when they are destroyed, and thus frees VRAM space for more tiles.
; Date: 2015-10-28 (Oct)
DisablePoints: = 0
; Notes: This makes the SYZ bumpers boring.

; Name: Store Level BGM
; Function: Stores the current level BGM for later use so we don't have to jump to MusicList every time.
; Date: 2015-10-28 (Oct)
StoreLevelBGM: = 1

; Name: Different BGM per Act
; Function: Allows each act to have a different BGM.
; Date: 2015-10-30 (Oct)
DynamicBGM: = 1
; Notes: It is recommended that you also turn on StoreLevelBGM with this fix.

; Name: "Act Completed" Pose
; Function: Makes Sonic do a pose after the end signpost, a la Sonic 3.
; Date: 2015-10-28 (Oct)
CompletePose: = 0
; Notes: Not completely done - code is there but pose is missing.

; Name: Eggman Monitor Function
; Function: Makes the Eggman monitor functional.
; Date: 2015-10-30 (Oct)
EggmanMonitor: = 1

; Name: Disable Level Select
; Function: Disables access to level select.
; Date: 2015-10-31 (Oct)
LevSelDisabled: = 0

; Name: Level Select Music
; Function: Plays some music (Sonic 2 Options theme by default) on the level select.
; Date: 2015-10-30 (Oct)
LevSelMus: = 1

; Name: S2B Level Select Features
...
; Name: Fade to Level Select
; Function: Makes the title screen fade out & in when going to level select, makes the transition look smoother.
; Date: 2015-11-4 (Nov)
FadeToLevSel: = 1

; Name: Level Select Header
; Function: Adds a title (" ==  LEVEL  SELECT  == " by default) to the level select screen.
; Date: 2015-11-6 (Nov)
LevSelHeader: = 1

; Name: Ending/Credits Sound Test Cheat
; Function: Enables/disables accessing the ending and the credits by playing specific sounds in the Sound Test.
; Date: 2015-11-6 (Nov)
EndingCreditsSndTest: = 0

; Name: No HUD in Ending
; Function: Prevents the HUD from loading in the ending sequence.
; Date: 2015-11-7 (Nov)
NoHUDInEnding: = 1
...
; Name: Custom Random Seed
; Function: Allows you to change the RandomNumber subroutine's seed.
; Date: 2015-11-9 (Nov)
CustomRandomSeed: = 0
	RNGSeed: = $2A6D365A	; default is $2A6D365A
	
; Name: Improved NemDec & KosDec
; Function: Uses improved versions of the NemDec & KosDec subroutines.
; Date: 2015-11-11 (Nov)
ImprovedDec: = 1
...
; Name: Mid-air Roll
; Function: Gives Sonic the ability to roll into a ball in midair.
; Date: 2015-11-19 (Nov)
MidairRollActive: = 1

; Name: Rings From Enemies
; Function: Makes enemies spawn rings when destroyed.
; Date: 2015-11-19 (Nov)
RingsFromEnemies: = 1

; Name: Rings From Monitors
; Function: Makes monitors spawn rings when destroyed.
; Date: 2015-11-19 (Nov)
RingsFromMonitors: = 1
...
; Name: Song Restoration Fix
; Function: Fixes song restoration (DAC not returning when paused while fading in and SS missing the FM6 layer after extra life).
; Date: 2015-10-27 (Oct)
; Modifies: s1.sounddriver.asm

; Name: Seperated Start Position Files
; Function: Seperates the start position files (now found at startpos) so that the disassembly is compatible with SonED2.
; Date: 2015-10-29 (Oct)
; Modifies: _inc\LevelSizeLoad & BgScrollSpeed.asm, _inc\LevelSizeLoad & BgScrollSpeed (JP1).asm

; Name: Remember Sprite Fix
; Function: Fixes a bug that caused destroyed objects to reappear when object RAM is full.
; Date: 2015-10-29 (Oct)
; Modifies: sonic.asm

; Name: Accidental Ring Deletion Fix
; Function: Fixes a bug that caused rings to be deleted at the "bottom" (wrapping point) of vertically-wrapping levels.
; Date: 2015-10-29 (Oct)
; Modifies: _incObj\25 & 37 Rings.asm

; Name: Drowning Bugs Fix
; Function: Fixes several bugs related to drowning.
; Date: 2015-10-29 (Oct)
; Modifies: sonic.asm, _incObj\0A Drowning Countdown.asm

; Name: Level Select Graphics Fix
; Function: Fixes a rare bug that causes an incorrectly colored piece of GHZ's background to appear when level select is entered at a very specific time.
; Date: 2015-10-29 (Oct)
; Modifies: sonic.asm

; Name: Sound Driver Fixes
; Function: Fixes several bugs related to the sound driver.
; Date: 2015-11-5 (Nov)
; Modifies: sonic.asm, s1.sounddriver.asm

; Name: Game/Time Over Flicker Fix
; Function: Fixes the Game/Time Over text flickering when the two words cojoin.
; Date: 2015-11-5 (Nov)
; Modifies: _incObj/39 Game Over.asm

; Name: Garbled Bubbles When Drowning Fix
; Function: Fixes garbled bubbles being displayed when drowning sometimes.
; Date: 2015-11-5 (Nov)
; Modifies: _incObj/0A Drowning Countdown.asm

; Name: Wrong Way After Spring Fix
; Function: Fixes Sonic facing the wrong way (towards the spring instead of away from it) after hitting a spring.
; Date: 2015-11-5 (Nov)
; Modifies: _incObj/41 Springs.asm

; Name: Displaying Nonexistent Objects Fix
; Function: Fixes a bug that results from calling DisplaySprite right after DeleteObject.
; Date: 2015-11-5 (Nov)
; Modifies: A LOT OF ASM FILES IN _incObj

; Name: Debug Mode in Ending without Cheat
; Function: Fixes a silly bug where you could get debug mode in the ending cutscene without having the cheat active.
; Date: 2015-11-5 (Nov)
; Modifies: sonic.asm

; Name: Horizontal Scrolling Fix
; Function: Fixes some bugs related to horizontal scrolling.
; Date: 2015-11-7 (Nov)
; Modifies: _inc/DeformLayers.asm, _inc/DeformLayers (JP1).asm

; Name: Incorrect Height when Rolljumping Fix
; Function: Fixes a bug where the player has the wrong height value when rolljumping, which results in the player stopping in midair and preventing another jump.
; Date: 2015-11-8 (Nov)
; Modifies: _incObj/Sonic Jump.asm
...
v_anglebuffer_r:= v_anglebuffer+2	; rear angle of collision block that Sonic or object is standing on
...
v_centstep:	= $FFFFFEBF	; value used to increment centiseconds
	...
	if StoreLevelBGM=1
v_lvlbgm:	= $FFFFFF90	; stores the current level bgm
	endc
...
SegaPCMPitch:	= 19	; does what it says on the tin - controls the Sega PCM pitch

; MUSIC & SFX
; ================================================================================

	if LevSelMus=1
bgm_LvlSel:	equ ((ptr_mus94-MusicIndex)/4)+$81
	endc
