...
ReadJoypads:				; XREF: VBlank, HBlank
		...
		bsr.s	Jpad_Read	; do the first joypad
...
; End of function TilemapToVRAM
	...
	if ImprovedDec=1
		include	"_inc\Nemesis Decompression (Improved).asm"
	else
...
AddAnimalPLC:
		...
		move.b	(v_zone).w,d0
		cmpi.w	#id_NewZ,d0
		bhs.s	AddAnimalPLC_New
		addi.w	#$15,d0
		bra.s	AddPLC
; ---------------------------------------------------------------------------
 
AddAnimalPLC_New:
		subi.w	#7,d0	; multiply d0 by 3
		move.w	d0,d1
		add.w	d0,d0
		add.w	d1,d0	; add $22 (this is the index of the animal PLC for the first added zone)
		addi.w	#$22,d0
; End of function AddAnimalPLC
	endc
...
; End of function QuickPLC
	...
	if ImprovedDec=1
		include	"_inc\Kosinski Decompression (Improved).asm"
	else
...
GM_Title:				; XREF: GameModeArray
	...
	Tit_ClrPal:
		...
		moveq	#palid_Sonic,d0	; load Sonic palette
		bsr.w	PalLoad1
	...
	if ChunksInROM=1	;Mercury Chunks In ROM
		copyTilemap	Eni_Title,$C208,$21,$15
	else
		lea	($FF0000).l,a1
...
Tit_EnterCheat:
	...
	if SimpleCheatEntry=1	;Mercury Simple Cheat Entry
		...
		if CheatFlash=1
		move.b	#sfx_EnterSS,d0	; play SS enter sound when code is entered
		bsr.w	PlaySound_Special
		movem.l	d0-a6,-(sp)
		...
		if CheatFlash=1
		move.b	#sfx_Teleport,d0	; play teleport sound when cheat is activated
		bsr.w	PlaySound_Special
		movem.l	d0-a6,-(sp)
		bsr.w	PaletteWhiteIn
		movem.l	(sp)+,d0-a6
		else
		move.b	#sfx_Ring,d0	; play ring sound when cheat is activated
		bsr.w	PlaySound_Special
		endc
...
Tit_ChkLevSel:
	...
	if LevSelMus=1
		move.b	#bgm_LvlSel,d0
		jsr	PlaySound
		moveq	#0,d0	; reset d0 just in case
	endc
...
LevelSelect:
	if S2BLevSel=1
		move.b	#2,(v_vbla_routine).w	; changed from #4 to fix graphics bug
		bsr.w	WaitForVBla
	...
	if SndTstAScroll=1
		cmpi.b	#btnA,(v_jpadpress1).w ; is A pressed?
...
LevSel_NoCheat:
		cmpi.w	#$94+LevSelMus,d0	; is sound $80-$94 being played?
		bcs.s	LevSel_PlaySnd	; if yes, branch
		cmpi.w	#$A0,d0		; is sound $95-$A0 being played?
		bcs.s	LevelSelect	; if yes, branch
...
LevSel_Ptrs:	dc.b id_GHZ, 0
		...
		dc.b id_SBZ, 0
		dc.b id_SBZ, 1
		dc.b id_LZ, 3	; LZ4 is SBZ3
		dc.b id_SBZ, 2	; SBZ3 is FZ
...
LevSelCode_J:	dc.b btnA,0,$FF
		even

LevSelCode_US:	dc.b btnA,0,$FF
		even
	endc
; ===========================================================================
...
LevSel_SndTest:				; XREF: LevSelControls
		...
		move.b	(v_jpadpress1).w,d1
		andi.b	#btnR+btnL+btnA*SndTstAScroll,d1	; is left/right pressed?
...
LevSelTextLoad:				; XREF: GM_Title
	...
	if LevSelHeader=1
		move.l	d4,-(sp)
		move.l	a1,-(sp)
		move.l	#textpos-$800000,d4	; text position on screen
		lea	(LevelMenuHeader).l,a1
		move.l	d4,4(a6)
...
; ---------------------------------------------------------------------------
	if DynamicBGM=1
; ---------------------------------------------------------------------------
; Subroutine to load the correct music list
; Output:
;	a1 = music list
; ---------------------------------------------------------------------------
GetMusicList:
...
@act1:		dc.b bgm_GHZ, bgm_LZ, bgm_MZ, bgm_SLZ, bgm_SYZ, bgm_SBZ
		even
...
Level_GetBgm:
	...
	endc
		move.b	(a1,d0.w),d0
	if StoreLevelBGM=1
		move.w	d0,(v_lvlbgm).w	; store level music
	endc
...
Level_ClrCardArt:
	...
	if NewZonePatches=1
		...
		move.b	(v_zone).w,d0
		addi.w	#$15,d0
		jsr	(AddPLC).l	; load animal patterns (level no. + $15)
	endc
...
Sonic_Main:	; Routine 0
	...
	if (SpinDashDustActive|InstaShieldActive)=1	;Mercury Spin Dash Dust, Insta-Shield
		move.b	#id_SonicEffects,(v_objspace+$1C0).w	; load effects object
	endc	;end Spin Dash Dust, Insta-Shield
...
@contMode:
	...
	if Revision=0
		include	"_incObj\Sonic Squash.asm"
	endc
...
ResumeMusic:				; XREF: Obj64_Wobble; Sonic_Water; Obj0A_ReduceAir
	...
	if StoreLevelBGM=1
		move.w	(v_lvlbgm).w,d0	; get level music
	else
		moveq	#0,d0
		move.b	(v_zone).w,d0
		if DynamicBGM=1
...
; ===========================================================================
		...
		include	"_incObj\sub FloorLog.asm"
...
SS_LoadWalls:
	...
	@return:
		rts
	endc	;end Mercury Dynamic Special Stage Walls
...
SS_Load:				; XREF: GM_Special
	...
	if SpecialStageIndexIncreasesOnlyIfWon=1	;Mercury Special Stage Index Increases Only If Won
		cmpi.b	#6,d0	; is last special stage the last one?
		bcs.s	SS_ChkEmldNum	; if not, branch
		move.b	#0,d0	; reset to first special stage
...
loc_1B6F8:
	...
	if SpecialStagesStillAppearWithAllEmeralds=1	;Mercury Special Stages Still Appear With All Emeralds
		...
		bcs.s	@notem	; if not, branch
		cmpi.b	#$40,d0	; is this item an emerald (dupe)?
		bhi.s	@notem	; if not, branch
		cmpi.b	#6,(v_emeralds).w	; do you have all the emeralds?
		bne.s	@notem		; if not, branch
		move.b	#$28,d0		; make a 1up
...
; ---------------------------------------------------------------------------
	...
	if DynamicSpecialStageWalls=1	;Mercury Dynamic Special Stage Walls
Art_SSWalls:	incbin	"artnem_u\(Mercury) Special Walls (dynamic).bin" ; special stage walls
		even
	else
...
Blk16_Title:	incbin	"map16_u\Title.bin"
	else
...
Blk16_Title:	incbin	"map16\Title.bin"
	endc	;end Blocks In ROM
		...
		even
Nem_Title:	incbin	"artnem\8x8 - Title.bin"	; TS primary patterns
		even

	if S2ArtLoader=1
		if CompInsteadOfKos=1
Comp_GHZ:	incbin	"artcomp\8x8 - GHZ.bin"	; GHZ primary patterns
		else
...
Blk16_LZ:	incbin	"map16\LZ.bin"
	...
	if S2ArtLoader=1
		if CompInsteadOfKos=1
Comp_LZ:	incbin	"artcomp\8x8 - LZ.bin"	; LZ primary patterns
		else
...
Blk16_MZ:	incbin	"map16\MZ.bin"
	...
	if S2ArtLoader=1
		if CompInsteadOfKos=1
Comp_MZ:	incbin	"artcomp\8x8 - MZ.bin"	; MZ primary patterns
		else
...
Blk16_SLZ:	incbin	"map16\SLZ.bin"
	...
	if S2ArtLoader=1
		if CompInsteadOfKos=1
Comp_SLZ:	incbin	"artcomp\8x8 - SLZ.bin"	; SLZ primary patterns
		else
...
Blk16_SYZ:	incbin	"map16\SYZ.bin"
	...
	if S2ArtLoader=1
		if CompInsteadOfKos=1
Comp_SYZ:	incbin	"artcomp\8x8 - SYZ.bin"	; SYZ primary patterns
		else
Kos_SYZ:	incbin	"artkos\8x8 - SYZ.bin"	; SYZ primary patterns
		endc
	else
Nem_SYZ:	incbin	"artnem\8x8 - SYZ.bin"	; SLZ primary patterns
	endc
		even
...
Blk16_SBZ:	incbin	"map16\SBZ.bin"
	...
	if S2ArtLoader=1
		if CompInsteadOfKos=1
Comp_SBZ:	incbin	"artcomp\8x8 - SBZ.bin"	; SBZ primary patterns
		else
...
; ---------------------------------------------------------------------------
	...
	include	"s1.sounddriver.asm"	; includes the sound driver
...
EndOfRom:
	...
	if LogROMSize=1
		if PaddingActive=1
		inform	0,"ROM size is $%h.",EndOfRom
		inform	0,"About $%h of it is padding.",Padding
		else
		inform	0,"ROM size is $%h.",EndOfRom
		endc
	endc
